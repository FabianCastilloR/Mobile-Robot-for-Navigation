...
This code sends the coordinates (x, y) in cm of the robot and goal to the server. The code works through artificial vision. 
The user must define a ROI (Region of Interest) in real life and the camera must be visualizing it. The code will automatically 
detect the ROI and the frame will be delimited to it. This code also allows having an inclination of the camera with respect to 
the ground because it uses the perspective transform to align the image. The user must define the width and height of the ROI in 
px (resolution) and the real width and height of the ROI in cm. The user must also define the IP and port of the server.
...
The user must define the HSV values of the robot and goal in the code. The user must also define the minimum area of the robot 
and goal that the user wants to detect.

This code works only with Arduino's server code "ESP32_Server_WNoObstacles.ino". To take into account only one obstacle, the user
...
def objDetection(imgAligned):
    ...
    return imgAligned, xRobCentroid, yRobCentroid, xObjCentroid, yObjCentroid, isRobot, isObject
    ...
    try:
        while(cap.isOpened()):
            ...
            if ret == True:
                ...
                # Detection of robot and objective and its centroids
                imgAligned, xRobCentroidPX, yRobCentroidPX, xObjCentroidPX, yObjCentroidPX, isRobot, isObject = objDetection(imgAligned)
                ...
                if isRobot and isObject:
                    ...
                    hxObjPX = xObjCentroidPX - imgAligned.shape[1] / 2
                    hyObjPX = imgAligned.shape[0] / 2 - yObjCentroidPX
                    hxObjCM = float(hxObjPX) * widthReal / widthROI
                    hyObjCM = float(hyObjPX) * heightReal / heightROI
                    hxRobErrorCM = hxObjCM - hxRobCM
                    hyRobErrorCM = hyObjCM - hyRobCM
                    hxLastErrorCM = hxRobErrorCM
                    hyLastErrorCM = hyRobErrorCM
                else:
                    hxLastErrorCM = 0
                    hyLastErrorCM = 0
                
                sock.send("{},{},{},{}".format(hxLastErrorCM, hyLastErrorCM, isRobot, isObject).encode())
